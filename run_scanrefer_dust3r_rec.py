...
def run_single_scene(
    ...
    image_root = os.path.join(cfg.paths.root_path, 
                              f'rec_{cfg.scan_type}_images', 
                              scene_id)
    ...
    gt_scene = load_scene(cfg.paths.root_path, 
                          scene_id=scene_id, sp_paths=super_points_path)
    gt_point_cloud = gt_scene['point_cloud']
    instance_mask = gt_scene['instance_mask']
    semantic_mask = gt_scene['semantic_mask']

    rec_prefix_data = dict(points=f'points_{cfg.scan_type}')
    rec_scene = load_scene(cfg.paths.root_path_points, 
                           scene_id=scene_id, 
                           sp_paths=super_points_path, 
                           data_prefix=rec_prefix_data)
    rec_point_cloud = rec_scene['point_cloud']
    ...
    for idx in range(len(prompt_arr)):
        ...
        _, mask_3d_list = multiple_img2pointcloud(
            ...
            write=False,
            data_prefix=dict(posed_images=f'rec_{cfg.scan_type}_images')
        )
            
        # Project 3D masks onto the point cloud with 1-NN
        final_bin_masks = project_masks2pcd(mask_3d_list, rec_point_cloud)
        ...
        if best_mask_id is not None:
            ...
            final_mask_3d = rec_point_cloud[ids]
            N = rec_point_cloud.shape[0]
            ...
            if len(final_bin_masks) > 0:
                ...
                final_mask_3d = rec_point_cloud[final_bin_mask.astype('bool')]
            else:
                final_mask_3d = None
